...
function shouldTraceMessage(event: SlackMessageEvent, botUserId?: string): boolean {
  // Skip bot messages
  if (event.bot_id) {
    console.log('🧠 [BETA-PRECHECK] Skipping bot message');
    return false;
  }
  
  // Skip messages with certain subtypes
  if (event.subtype && event.subtype !== 'bot_message') {
    console.log('🧠 [BETA-PRECHECK] Skipping subtype:', event.subtype);
    return false;
  }
  ...
  if (!messageText) {
    console.log('🧠 [BETA-PRECHECK] No message text');
    return false;
  }
  ...
  if (!mentions || mentions.length === 0) {
    console.log('🧠 [BETA-PRECHECK] No bot mentions found');
    return false;
  }
  
  // If we have a bot user ID, check if this bot is mentioned
  if (botUserId && !messageText.includes(`<@${botUserId}>`)) {
    console.log('🧠 [BETA-PRECHECK] This bot not mentioned');
    return false;
  }
  
  console.log('🧠 [BETA-PRECHECK] Message should be traced');
  return true;
}
  ...
  return {
    invoke: async (initialState: SlackWorkflowState) => {
      ...
      if (!shouldTraceMessage(event, botUserId)) {
        console.log('🧠 [BETA] Skipping trace for message that won\'t be processed');
      ...
      return traced(
        async (span: { log: (data: Record<string, unknown>) => void }) => {
          ...
          if (!result.shouldProcess) {
            console.log('🧠 [BETA-TRACE] Workflow determined not to process, skipping log');
            return result;
          }
